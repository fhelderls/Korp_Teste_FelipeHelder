...
Como a descrição e o preço do item ficam travados no momento da **criação** da nota (não no momento da emissão), uma nota `Aberta` sem limite de validade viraria uma forma de segurar um preço antigo indefinidamente — por exemplo, criar notas durante uma promoção relâmpago e só confirmar a emissão depois que o preço normal já voltou. Por isso existe um prazo de validade (`TTLReserva`, 7 dias, seguindo a mesma lógica de um orçamento comercial com prazo de validade) tratado em dois pontos:

- Na **confirmação**: se alguém tentar emitir uma nota cuja reserva já passou do prazo, o `estoque-service` recusa e cancela a reserva ali mesmo, em vez de aceitar um preço vencido.
- Na **listagem**: toda vez que a tela de Notas carrega (`GET /notas`), o `faturamento-service` varre as notas `Aberta` vencidas e as expira de verdade — chama `POST /reservas/{chave}/cancelar` no `estoque-service` pra liberar a reserva (essa chamada é idempotente: cancelar uma reserva que já tinha sido cancelada pelo passo anterior não é erro) e só então marca a nota como **`Cancelada`**. A ordem importa: a reserva só é liberada e confirmada primeiro, a nota só muda de status depois — assim nunca existe uma nota `Cancelada` com estoque ainda preso do outro lado. Se a liberação falhar (`estoque-service` fora do ar), a nota continua `Aberta` por enquanto e a tentativa se repete sozinha na próxima vez que a lista for carregada. Não existe job em background: a expiração só acontece quando alguém efetivamente olha pra lista ou tenta emitir.

Se o `estoque-service` estiver fora do ar ou recusar a confirmação por falta de saldo, a nota **continua `Aberta`** (com a reserva ainda pendente, sem data de emissão), e o erro é devolvido ao usuário de forma clara. Não existe um status de "falha" separado para esses casos: uma nota que não foi emitida com sucesso simplesmente segue `Aberta`, e o usuário pode clicar em Emitir de novo quando quiser — essa é a própria forma de reprocessar, sem precisar de uma ação especial.

Uma nota `Aberta` também pode ser **cancelada manualmente** (`DELETE /notas/{chave}`) pelo usuário, se ele desistir dela: chama `POST /reservas/{chave}/cancelar` no `estoque-service` (que libera a reserva, sem precisar devolver saldo nenhum, já que reservar nunca descontou nada) e remove a nota por completo (diferente da expiração automática por TTL, que mantém a nota como `Cancelada` em vez de apagar, preservando o histórico). É a terceira ação do padrão Saga, ao lado de confirmar — sem ela, uma nota `Aberta` abandonada deixaria estoque preso pra sempre.
...
2. **Como ferramenta auxiliar no desenvolvimento**: o Claude Code foi usado principalmente para esclarecer dúvidas sobre conceitos novos (Go, Angular, Docker) ao longo do desenvolvimento, e para testar e revisar o projeto já finalizado antes da entrega.
